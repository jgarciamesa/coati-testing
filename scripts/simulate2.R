...
simulate_two_main = function(input, output) {
    ...
    # get origin (aligner)
    origin_id = sample.int(n = length(origins), size = 1)
    # get cigar string from that origin (aligner)
    cigar_id = sample(x = which(gaps_file$origin == origins[origin_id]), size = 1)
    cigar_id = cigar_id - 1
...
if(!interactive()) {
    ...
    origins = unique(gaps_file$origin)
